...
def agreement():
    if os.path.exists('./cache'):
        return
    if not messagebox.askyesno(title='使用需知', message='本项目Phigros Data Farmer由lemocn在Github开源免费发布。\n网址:https://github.com/lemocn/PhigrosDataFarmer\n如您通过购买取得本项目，请举报+退款。\n使用本程序所产生的一切后果将由您自己承担。\n' '您是否知情且同意？'):
        exit(1)
...
class App(ttk.Frame):
    ...
    def xizorun(self):
        try:
            ...
            # 增加 is_loop 参数，默认为 False
            def go_now(is_loop=False):
                def stop():
                    self.running = False
                ...
                def gogogo():
                    ...
                    if is_loop:
                        ...
                        time.sleep(0.1)
                        self.controller.tap(30, 30)
                        time.sleep(0.83)  # 等待暂停菜单完全展开
                        self.controller.tap(device_width >> 1, device_height >> 1) # 点击重新开始
                    else:
                        # 初始状态：从手动暂停界面直接开始
                        self.controller.tap(30, 30)
                        time.sleep(0.83)
                    ...
                    def stop_playing():
                        self.running = False
                    self.xizobtn['command'] = stop_playing
